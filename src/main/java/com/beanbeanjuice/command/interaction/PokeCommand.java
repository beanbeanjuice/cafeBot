...
public class PokeCommand implements ICommand {
    ...
    public void handle(CommandContext ctx, ArrayList<String> args, User user, GuildMessageReceivedEvent event) {
        String url = CafeBot.getInteractionHandler().getPokeImage();
        ...
        if (receivers.size() == 0) {
            message = "**" + sender + "** *poked* themselves! What are you trying to do?";
        } else {
            message = "**" + sender + "** *poked* **" + CafeBot.getInteractionHandler().getReceiverString(receivers) + "**!";

            if (receivers.size() == 1) {
                int sendAmount = CafeBot.getInteractionHandler().getSender(user.getId(), InteractionType.POKE) + 1;
                int receiveAmount = CafeBot.getInteractionHandler().getReceiver(receivers.get(0).getId(), InteractionType.POKE) + 1;

                CafeBot.getInteractionHandler().updateSender(user.getId(), InteractionType.POKE, sendAmount);
                CafeBot.getInteractionHandler().updateReceiver(receivers.get(0).getId(), InteractionType.POKE, receiveAmount);

                footer = user.getName() + " poked others " + sendAmount + " times. " + receivers.get(0).getName() + " was poked " + receiveAmount + " times.";
            }
        }
