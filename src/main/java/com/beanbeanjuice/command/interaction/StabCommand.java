...
public class StabCommand implements ICommand {
    ...
    public void handle(CommandContext ctx, ArrayList<String> args, User user, GuildMessageReceivedEvent event) {
        ...
        if (receivers.size() == 0) {
            message = "**" + sender + "** *stabbed* themselves! SOMEONE PLEASE HELP!";
        } else {
            message = "**" + sender + "** *stabbed* **" + CafeBot.getInteractionHandler().getReceiverString(receivers) + "**!";

            if (receivers.size() == 1) {
                int sendAmount = CafeBot.getInteractionHandler().getSender(user.getId(), InteractionType.STAB) + 1;
                int receiveAmount = CafeBot.getInteractionHandler().getReceiver(receivers.get(0).getId(), InteractionType.STAB) + 1;

                CafeBot.getInteractionHandler().updateSender(user.getId(), InteractionType.STAB, sendAmount);
                CafeBot.getInteractionHandler().updateReceiver(receivers.get(0).getId(), InteractionType.STAB, receiveAmount);

                footer = user.getName() + " stabbed others " + sendAmount + " times. " + receivers.get(0).getName() + " was stabbed " + receiveAmount + " times.";
            }
        }
