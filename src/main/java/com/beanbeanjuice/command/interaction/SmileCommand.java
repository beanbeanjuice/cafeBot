...
public class SmileCommand implements ICommand {
    ...
    public void handle(CommandContext ctx, ArrayList<String> args, User user, GuildMessageReceivedEvent event) {
        String url = CafeBot.getInteractionHandler().getSmileImage();
        ...
        if (receivers.size() == 0) {
            message = "**" + sender + "** *smiled* at themselves!";
        } else {
            message = "**" + sender + "** *smiled* at **" + CafeBot.getInteractionHandler().getReceiverString(receivers) + "**!";

            if (receivers.size() == 1) {
                int sendAmount = CafeBot.getInteractionHandler().getSender(user.getId(), InteractionType.SMILE) + 1;
                int receiveAmount = CafeBot.getInteractionHandler().getReceiver(receivers.get(0).getId(), InteractionType.SMILE) + 1;

                CafeBot.getInteractionHandler().updateSender(user.getId(), InteractionType.SMILE, sendAmount);
                CafeBot.getInteractionHandler().updateReceiver(receivers.get(0).getId(), InteractionType.SMILE, receiveAmount);

                footer = user.getName() + " smiled at others " + sendAmount + " times. " + receivers.get(0).getName() + " was smiled at " + receiveAmount + " times.";
            }
        }
