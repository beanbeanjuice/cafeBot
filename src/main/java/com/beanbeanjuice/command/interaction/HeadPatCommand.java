...
public class HeadPatCommand implements ICommand {
    ...
    public void handle(CommandContext ctx, ArrayList<String> args, User user, GuildMessageReceivedEvent event) {
        ...
        if (receivers.size() == 0) {
            message = "**" + sender + "** is *headpatting* themselves! That's honestly so sad...";
        } else {
            message = "**" + sender + "** *headpats* **" + CafeBot.getInteractionHandler().getReceiverString(receivers) + "**!";

            if (receivers.size() == 1) {
                int sendAmount = CafeBot.getInteractionHandler().getSender(user.getId(), InteractionType.HEADPAT) + 1;
                int receiveAmount = CafeBot.getInteractionHandler().getReceiver(receivers.get(0).getId(), InteractionType.HEADPAT) + 1;

                CafeBot.getInteractionHandler().updateSender(user.getId(), InteractionType.HEADPAT, sendAmount);
                CafeBot.getInteractionHandler().updateReceiver(receivers.get(0).getId(), InteractionType.HEADPAT, receiveAmount);

                footer = user.getName() + " headpat others " + sendAmount + " times. " + receivers.get(0).getName() + " was headpat " + receiveAmount + " times.";
            }
        }
