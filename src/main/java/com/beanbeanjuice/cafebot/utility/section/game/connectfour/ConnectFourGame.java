...
public class ConnectFourGame {
    ...
    private void stopGameTimer() {
        gameTimer.cancel();
        ConnectFourHandler.stopGame(this);
     ...
     * @return True, if the turn can be run.
     */
    @NotNull
    ...
    private void winGame(@NotNull User user) {
        ...
        Integer currentWinStreak = WinStreakHandler.getUserWinStreak(user.getId(), MinigameType.CONNECT_FOUR);
        User loser;
        ...
        Integer loserWinStreak = WinStreakHandler.getUserWinStreak(loser.getId(), MinigameType.CONNECT_FOUR);

        if (currentWinStreak != null) {
            if (!WinStreakHandler.updateUserWinStreak(user.getId(), MinigameType.CONNECT_FOUR, ++currentWinStreak))
                currentWinStreak = null;

            if (!WinStreakHandler.updateUserWinStreak(loser.getId(), MinigameType.CONNECT_FOUR, 0))
                loserWinStreak = null;
        }
