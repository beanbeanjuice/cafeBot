...
public class DefineCommand extends Command implements ICommand {
    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        DictionaryAPIWrapper dictionaryAPIWrapper = new DictionaryAPIWrapper(cafeBot);
        dictionaryAPIWrapper.getDictionaryEmbed(word, languageCode)
                ...
                .exceptionallyAsync((ignored) -> {
                    event.getHook().sendMessageEmbeds(Helper.errorEmbed(
                            "Error Defining Word",
                            "There might be an error with the API or the word just simply might not exist."
                    )).queue();
                    return null;
                });
    }
