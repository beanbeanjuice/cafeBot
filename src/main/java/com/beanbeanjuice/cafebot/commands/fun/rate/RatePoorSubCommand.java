...
public class RatePoorSubCommand extends Command implements ISubCommand {
    ...
    public void handle(SlashCommandInteractionEvent event, CommandContext ctx) {
        ...
        String description = ctx.getGuildI18n().getString("command.rate.subcommand.poor.embed.description")
                .replace("{user}", user.getAsMention())
                .replace("{percent}", String.valueOf(percentage));

        event.getHook().sendMessageEmbeds(Helper.successEmbed(
                ctx.getGuildI18n().getString("command.rate.subcommand.poor.embed.title"),
                description
        )).mention(user).queue();
    }
    ...
    public String getDescriptionPath() {
        return "command.rate.subcommand.poor.description";
    }

    @Override
    public OptionData[] getOptions() {
        return new OptionData[] {
                new OptionData(OptionType.USER, "user", "command.rate.subcommand.poor.arguments.user.description")
        };
    }
