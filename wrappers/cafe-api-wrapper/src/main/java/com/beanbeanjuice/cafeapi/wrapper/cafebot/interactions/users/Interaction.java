package com.beanbeanjuice.cafeapi.wrapper.cafebot.interactions.users;

import lombok.Getter;

/**
 * A class used to hold {@link Interaction} information.
 *
 * @author beanbeanjuice
 */
@Getter
public class Interaction {

    // TODO: Switch to a hash map jesus christ.
    private final int hugAmount;
    private final int punchAmount;
    private final int kissAmount;
    private final int biteAmount;
    private final int blushAmount;
    private final int cuddleAmount;
    private final int nomAmount;
    private final int pokeAmount;
    private final int slapAmount;
    private final int stabAmount;
    private final int hmphAmount;
    private final int poutAmount;
    private final int throwAmount;
    private final int smileAmount;
    private final int stareAmount;
    private final int tickleAmount;
    private final int rageAmount;
    private final int yellAmount;
    private final int headpatAmount;
    private final int cryAmount;
    private final int danceAmount;
    private final int dabAmount;
    private final int bonkAmount;
    private final int sleepAmount;
    private final int dieAmount;
    private final int welcomeAmount;
    private final int lickAmount;
    private final int shushAmount;
    private final int waveAmount;
    private final int shootAmount;
    private final int amazedAmount;
    private final int askAmount;
    private final int boopAmount;
    private final int loveAmount;
    private final int nosebleedAmount;
    private final int okAmount;
    private final int UWUAmount;
    private final int winkAmount;

    /**
     * Creates a new {@link Interaction object.}
     * @param hugAmount The {@link Integer hugAmount}.
     * @param punchAmount The {@link Integer punchAmount}.
     * @param kissAmount The {@link Integer kissAmount}.
     * @param biteAmount The {@link Integer biteAmount}.
     * @param blushAmount The {@link Integer blushAmount}.
     * @param cuddleAmount The {@link Integer cuddleAmount}.
     * @param nomAmount The {@link Integer nomAmount}.
     * @param pokeAmount The {@link Integer pokeAmount}.
     * @param slapAmount The {@link Integer slapAmount}.
     * @param stabAmount The {@link Integer stabAmount}.
     * @param hmphAmount The {@link Integer hmphAmount}.
     * @param poutAmount The {@link Integer poutAmount}.
     * @param throwAmount The {@link Integer throwAmount}.
     * @param smileAmount The {@link Integer smileAmount}.
     * @param stareAmount The {@link Integer stareAmount}.
     * @param tickleAmount The {@link Integer tickleAmount}.
     * @param rageAmount The {@link Integer rageAmount}.
     * @param yellAmount The {@link Integer yellAmount}.
     * @param headpatAmount The {@link Integer headpatAmount}.
     * @param cryAmount The {@link Integer cryAmount}.
     * @param danceAmount The {@link Integer danceAmount}.
     * @param dabAmount The {@link Integer dabAmount}.
     * @param bonkAmount The {@link Integer bonkAmount}.
     * @param sleepAmount The {@link Integer sleepAmount}.
     * @param dieAmount The {@link Integer dieAmount}.
     * @param welcomeAmount The {@link Integer welcomeAmount}.
     * @param lickAmount The {@link Integer lickAmount}.
     * @param shushAmount The {@link Integer shushAmount}.
     * @param waveAmount The {@link Integer waveAmount}.
     * @param shootAmount The {@link Integer shootAmount}.
     * @param amazedAmount The {@link Integer amazedAmount}.
     * @param askAmount The {@link Integer askAmount}.
     * @param boopAmount The {@link Integer boopAmount}.
     * @param loveAmount The {@link Integer loveAmount}.
     * @param nosebleedAmount The {@link Integer nosebleedAmount}.
     * @param okAmount The {@link Integer okAmount}.
     * @param UWUAmount The {@link Integer uwuAmount}.
     * @param winkAmount The {@link Integer winkAmount}.
     */
    public Interaction(int hugAmount, int punchAmount, int kissAmount,
                       int biteAmount, int blushAmount, int cuddleAmount,
                       int nomAmount, int pokeAmount, int slapAmount,
                       int stabAmount, int hmphAmount, int poutAmount,
                       int throwAmount, int smileAmount, int stareAmount,
                       int tickleAmount, int rageAmount, int yellAmount,
                       int headpatAmount, int cryAmount, int danceAmount,
                       int dabAmount, int bonkAmount, int sleepAmount,
                       int dieAmount, int welcomeAmount, int lickAmount,
                       int shushAmount, int waveAmount, int shootAmount,
                       int amazedAmount, int askAmount, int boopAmount,
                       int loveAmount, int nosebleedAmount, int okAmount,
                       int UWUAmount, int winkAmount) {
        this.hugAmount = hugAmount;
        this.punchAmount = punchAmount;
        this.kissAmount = kissAmount;
        this.biteAmount = biteAmount;
        this.blushAmount = blushAmount;
        this.cuddleAmount = cuddleAmount;
        this.nomAmount = nomAmount;
        this.pokeAmount = pokeAmount;
        this.slapAmount = slapAmount;
        this.stabAmount = stabAmount;
        this.hmphAmount = hmphAmount;
        this.poutAmount = poutAmount;
        this.throwAmount = throwAmount;
        this.smileAmount = smileAmount;
        this.stareAmount = stareAmount;
        this.tickleAmount = tickleAmount;
        this.rageAmount = rageAmount;
        this.yellAmount = yellAmount;
        this.headpatAmount = headpatAmount;
        this.cryAmount = cryAmount;
        this.danceAmount = danceAmount;
        this.dabAmount = dabAmount;
        this.bonkAmount = bonkAmount;
        this.sleepAmount = sleepAmount;
        this.dieAmount = dieAmount;
        this.welcomeAmount = welcomeAmount;
        this.lickAmount = lickAmount;
        this.shushAmount = shushAmount;
        this.waveAmount = waveAmount;
        this.shootAmount = shootAmount;
        this.amazedAmount = amazedAmount;
        this.askAmount = askAmount;
        this.boopAmount = boopAmount;
        this.loveAmount = loveAmount;
        this.nosebleedAmount = nosebleedAmount;
        this.okAmount = okAmount;
        this.UWUAmount = UWUAmount;
        this.winkAmount = winkAmount;
    }

}
