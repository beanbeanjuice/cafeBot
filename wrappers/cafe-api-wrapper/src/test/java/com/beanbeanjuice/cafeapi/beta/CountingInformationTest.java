...
public class CountingInformationTest {
    ...
    public void testCountingEndpoint() {
        CafeAPI cafeAPI = new CafeAPI("beanbeanjuice", System.getenv("API_PASSWORD"), RequestLocation.BETA);

        // Makes sure the counting information for the guild is deleted beforehand.
        Assertions.assertTrue(cafeAPI.getCountingEndpoint().deleteGuildCountingInformation("605489113323536433"));

        // Makes sure the counting information for the guild does not exist.
        Assertions.assertThrows(NotFoundException.class, () -> cafeAPI.getCountingEndpoint().getGuildCountingInformation("605489113323536433"));

        // Makes sure the counting information for the guild can be created.
        Assertions.assertTrue(cafeAPI.getCountingEndpoint().createGuildCountingInformation("605489113323536433"));

        // Makes sure duplicate counting informations cannot be created.
        Assertions.assertThrows(ConflictException.class, () -> cafeAPI.getCountingEndpoint().createGuildCountingInformation("605489113323536433"));

        // Makes sure the counting information's highest number by default is 0.
        Assertions.assertEquals(0, cafeAPI.getCountingEndpoint().getGuildCountingInformation("605489113323536433").getHighestNumber());

        // Makes sure the counting information's last number by default is 0.
        Assertions.assertEquals(0, cafeAPI.getCountingEndpoint().getAllCountingInformation().get("605489113323536433").getLastNumber());

        // Makes sure the counting information's last user ID by default is "0".
        Assertions.assertEquals("0", cafeAPI.getCountingEndpoint().getGuildCountingInformation("605489113323536433").getLastUserID());
        ...
        Assertions.assertEquals(133, cafeAPI.getCountingEndpoint().getGuildCountingInformation("605489113323536433").getHighestNumber());

        // Confirms the new last number is 100.
        Assertions.assertEquals(100, cafeAPI.getCountingEndpoint().getGuildCountingInformation("605489113323536433").getLastNumber());

        // Confirms the new last user ID is "178272524533104642".
        Assertions.assertEquals("178272524533104642", cafeAPI.getCountingEndpoint().getGuildCountingInformation("605489113323536433").getLastUserID());

        // Confirms the failure role ID is "165193414181126145"
        Assertions.assertEquals("165193414181126145", cafeAPI.getCountingEndpoint().getGuildCountingInformation("605489113323536433").getFailureRoleID());
